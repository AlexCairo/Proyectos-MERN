...
function PedidoPage(){
    ...
    const handleDeleteDetail = (index) => {
        ...
        console.log('detallePlato=> ', detallePlato);
        const nuevoDetalle = datos.detalle.filter((item, indexItem)=>indexItem!==index);
        console.log('nuevoDetalle=> ', nuevoDetalle);
        const total = datos.total - detallePlato.precio;
        const nDatos = {...datos, total, detalle: nuevoDetalle};
        setDatos(nDatos);
    ...
    const handleSubmit = async (e) => {
    	...
    	socket.emit('channel-cocina-pedido','listar');
    	await listarPedidos();
    	handleCloseModal();
    };
    ...
    return(
        <div className="container">
            <div className="d-flex justify-content-between mt-3">
                <h1>Pedidos</h1>
                <span>
                    <Button variant="primary" onClick={handleShowModal}>Nuevo</Button>
                </span>
            </div>
            <table className='table'>
                ...
                {listaPedidos.map(item=>(
                    <tr key={item._id}>
                        ...
                        <td>{item.estado}</td>
                        <td>
                            <Button onClick={()=>handleShowData(item._id)} variant="info" size="sm">
                        ...
                        <div className='row'>
                            <div className='col-md-8'>
                                <Dropdown onSelect={(eventKey)=>handleAddDetail(eventKey)}>
                                    ...
                                    <Dropdown.Menu as={CustomMenu}>
                                        ...
                                        <Dropdown.Item key={plato._id} eventKey={plato._id}>{plato.nombre} = S/{plato.precio}</Dropdown.Item>
                                        ))}
                                    </Dropdown.Menu>
                                </Dropdown>
                                <table className='table'>
                                    ...
                                    {datos.detalle.map((item, index)=>(
                                        <tr key={index}>
                                            ...
                                            <td>{item.estado}</td>
                                            <td>
                                                <Button onClick={()=>handleDeleteDetail(index)} variant="danger" size="sm">
                            ...
                            <div className='col-md-4'>
                                ...
                                <div className='mb-3'>
                                    ...
                                    <input type="text" onChange={handleChange} name="mesa" className='form-control' value={datos.mesa} required/>
                                </div>
                                <div className='mb-3'>
                                    <label className='form-label'>Cliente</label>
                                    <input type="text" onChange={handleChange} name="cliente" className='form-control' value={datos.cliente} required/>
                                </div>
                                <div className='mb-3'>
                                    <label className='form-label'>Estado</label>
                                    <select onChange={handleChange} className='form-select' name='estado' value={datos.estado} required>
